    ...
    while index < args.len() {
        ...
        let consumed = match value {
            ...
            value if value.starts_with("--") => {
                return Err(reject_unknown_option(value).expect_err("unknown option must error"));
            }
            value => {
                if subject.is_none() {
                    subject = Some(parse_subject(value)?);
...
// Tests locaux: le parseur d'options communes est un helper interne des
// commandes de service; les scenarios publics restent dans crates/app/tests.
#[cfg(test)]
mod tests {
    use super::{
        parse_required_path, parse_required_path_with_optional_named_path,
        parse_with_common_options,
    };
    use crate::cli::ParseOutcome;
    use crate::codex::ReasoningEffort;
    use crate::service_command::{RequiredPathParseSpec, ServiceCommandOptions};
