...
// Tests locaux: la preparation de contexte assemble des structures internes
// de service_command et ne doit pas agrandir la facade publique du crate.
#[cfg(test)]
mod tests {
    use super::{prepare_prompted_service, prepare_service_command};
    use crate::service_command::{ServiceCommandDescriptor, ServiceCommandOptions};
    ...
    fn prepare_service_command_uses_workspace_root_as_codex_working_directory() {
        ...
        let command =
            prepare_service_command(&options, &context, descriptor(), "Prompt".to_string());
